...
void		print_l_f(char *str, char *file, t_flags *toggle)
{
	...
	bits = perms(items.st_mode, toggle);
	ft_printf("%s", bits);
	suffix(file);
	...
	if ((toggle->i = (toggle->fuid - ft_strlen(user.pw_name))) > 0)
		print_spacing(toggle, str);
	ft_printf("%s  %s   ", user.pw_name, group.gr_name);
	...
	{
		if ((toggle->i = (toggle->fmajor - ft_numlen(major(items.st_rdev)))) > 0)
			print_spacing(toggle, str);
		ft_printf("%lu, ", major(items.st_rdev));
		if ((toggle->i = (toggle->fminor - ft_numlen(minor(items.st_rdev)))) > 0)
			print_spacing(toggle, str);
		ft_printf("%lu  ", minor(items.st_rdev));
	}
...
void		print_l(char *str, char *dir, char *file, t_flags *toggle)
{
	...
	lstat(file, &items);
	// if (items.st_size == 12288 || items.st_size == 4316 || items.st_size == 2497
	// 	|| items.st_size == 308440)
	// 	return ;
	user = *getpwuid(items.st_uid);
	group = *getgrgid(items.st_gid);
	bits = perms(items.st_mode, toggle);
	...
	{
		if ((toggle->i = (toggle->major - ft_numlen(major(items.st_rdev)))) > 0)
			print_spacing(toggle, str);
		ft_printf("%lu, ", major(items.st_rdev));
		if ((toggle->i = (toggle->minor - ft_numlen(minor(items.st_rdev)))) > 0)
			print_spacing(toggle, str);
		ft_printf("%lu  ", minor(items.st_rdev));
	}
	print_date_name(toggle, str, items, file);
