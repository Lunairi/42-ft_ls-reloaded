...
void	sort_hub(char **search, int exist, t_flags *toggle, int i)
{
	...
	if (toggle->t == 1 && toggle->re == 0)
		file_dir_time_sort(search, toggle->exist, toggle, 0);
	if (toggle->r == 1 && toggle->re == 0)
...
void	grab_format_file(char *str, char *file, t_flags *toggle)
{
	...
	if (S_ISREG(items.st_mode))
	{
		...
		if (ft_numlen(major(items.st_rdev)) > toggle->fmajor)
			toggle->fmajor = ft_numlen(major(items.st_rdev));
		if (ft_numlen(minor(items.st_rdev)) > toggle->fminor)
			toggle->fminor = ft_numlen(minor(items.st_rdev));
	}
}
