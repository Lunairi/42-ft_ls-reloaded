...
void	grab_format_long(char *str, char *dir, char *file, t_flags *toggle)
{
	...
	if (ft_numlen(major(items.st_rdev)) > toggle->major)
		toggle->major = ft_numlen(major(items.st_rdev));
	if (ft_numlen(minor(items.st_rdev)) > toggle->minor)
		toggle->minor = ft_numlen(minor(items.st_rdev));
}
